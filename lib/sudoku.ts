...
function createPuzzle(solvedBoard: number[][], difficulty: "easy" | "medium" | "hard"): SudokuBoard {
  ...
  for (let i = 0; i < cellsToRemove; i++) {
    ...
    // For hard puzzles, we don't check for uniqueness to allow multiple solutions
    // For easy and medium, we could check for uniqueness but it's computationally expensive
    // So we'll just ensure the puzzle is solvable
  }
