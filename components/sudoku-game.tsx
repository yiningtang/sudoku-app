    ...
    <div className="flex flex-col items-center">
      ...
      <div className="flex justify-between w-full mb-4">
        <div className="flex items-center space-x-2">
          <Button variant="outline" size="sm" onClick={() => setIsRunning(!isRunning)} className="flex items-center">
            <Clock className="h-4 w-4 mr-1" />
            {formatTime(timer)}
          </Button>
          <Button variant="outline" size="sm" onClick={newGame} className="flex items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button variant="outline" size="sm" onClick={getHint} className="flex items-center">
            ...
            {row.map((cell, colIndex) => (
              <div
                ...
                className={cn(
                  ...
                  !isCellSelected(rowIndex, colIndex) && isCellInSameRowOrCol(rowIndex, colIndex) && "bg-blue-50",
                  !isCellSelected(rowIndex, colIndex) && isCellInSameBlock(rowIndex, colIndex) && "bg-blue-50",
                ...
                {cell !== null ? (
                  ...
                  <div className="grid grid-cols-3 gap-0 w-full h-full p-0.5">
                    {notes[`${rowIndex}-${colIndex}`]?.map((note) => (
                      <span key={note} className="text-[8px] text-gray-500 flex items-center justify-center">
                        {note}
                      </span>
                    ))}
                  </div>
      ...
      {/* Clear button */}
      <Button variant="outline" className="w-full max-w-xs" onClick={handleClear}>
        Clear
      </Button>
